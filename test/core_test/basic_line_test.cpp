...
#include "core/basic_lines_cache.hpp"
...
TEST(basic_line, constructor_with_parameters)
{
    ...
    using lines_cache = cc::basic_lines_cache<char, 4, 4, 8, 4, 8, std::allocator<int>>;
    
    lines_cache lines_cche(0);
    chatacters_buffer_cache characters_buffer_cche(0);
    
    lines_cche.insert(0, line(0, EMPTY, EMPTY, &lines_cche, &characters_buffer_cche));
